...
// –азмеры диспле€ в пиксел€х
uint8_t SSD1306_Height;
uint8_t SSD1306_Width;


//==============================================================================
// ѕроцедура передаЄт команду с аргументами в ssd1306
//==============================================================================
void SSD1306_SendCommand(uint8_t Cmd, uint8_t *pBuff, uint16_t BuffLen)
{
  ...
  SSD1306_CS_HIGH();
#else   
  // ќстальные интерфейсы работы с ssd1306 пока не поддерживаютс€
#endif
}
//==============================================================================
...
//==============================================================================
// ѕроцедура передаЄт массив данных в ssd1306
//==============================================================================
...
//==============================================================================
// ѕроцедура устанавливает режим автосдвига указател€ в буфере кадра ssd1306
//==============================================================================
...
//==============================================================================
// ѕроцедура устанавливает страницу дл€ режима страничной адрессациии, когда
// сдвиг указател€ в пам€ти делаетс€ только по горизонтали (SSD1306_Adressing_Page).
//==============================================================================
...
//==============================================================================
// ѕроцедура устанавливает є строки, по которой указатель в буфере кадра будет перемещатьс€
// “олько дл€ режима автосдвига указател€ (SSD1306_Adressing_Page)
//==============================================================================
...
//==============================================================================
// —мещение по вертикали области отображени€ относительно пам€ти кадра
//==============================================================================
...
//==============================================================================
// Ќаправление сканировани€
// Value=0: normal mode (RESET) Scan from COM0 to COM[N Ц1]
...
//==============================================================================
// ѕроцедура отключает скролл (средствами контроллера диспле€
//==============================================================================
...
//==============================================================================
// ѕроцедура настройки ножек микроконтроллера дл€ обмена с дисплеем
//==============================================================================
...
#if (SSD1306_ResetPinUsed)      // ¬ключено управление ножкой reset ssd1306
...
#if (SSD1306_IfMode == SSD1306_IfMode_SPI_4w)       // –аботаем по SPI-4w
...
void SSD1306_Init(uint8_t Width, uint8_t Height)
{
  ...
  // —брос контроллера диспле€ ssd1306 ножкой Reset
#if (SSD1306_ResetPinUsed)      // ¬ключено управление ножкой reset ssd1306
  SSD1306_RESET_HIGH();
  delay_ms(2);
  SSD1306_RESET_LOW();  // –он€ем ножку reset в 0 на 10 мс
  delay_ms(15);
  SSD1306_RESET_HIGH();
#endif
  
  // ЎлЄм команды инициализации ssd1306
  SSD1306_Sleep();
  ...
  SSD1306_SetSegmentRemap(0);           // *мен€ет направление заполнение матрицы из буфера кадра (вертикаль/горизонталь)
