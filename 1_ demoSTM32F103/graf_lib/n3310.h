...
#define LCD_SDIN_PIN			GPIO_Pin_4  ///
#define LCD_SCLK_PIN			GPIO_Pin_3 ////

#define SET_DC					GPIO_SetBits(LCD_PORT, LCD_DC_PIN)
#define RESET_DC				GPIO_ResetBits(LCD_PORT, LCD_DC_PIN)

#define SET_CE					GPIO_SetBits(LCD_PORT, LCD_CE_PIN)
#define RESET_CE				GPIO_ResetBits(LCD_PORT, LCD_CE_PIN)

#define SET_RST					GPIO_SetBits(LCD_PORT, LCD_RST_PIN)
#define RESET_RST				GPIO_ResetBits(LCD_PORT, LCD_RST_PIN)

#define SET_SDIN				GPIO_SetBits(LCD_PORT, LCD_SDIN_PIN)
#define RESET_SDIN				GPIO_ResetBits(LCD_PORT, LCD_SDIN_PIN)

#define SET_SCLK				GPIO_SetBits(LCD_PORT, LCD_SCLK_PIN)
#define RESET_SCLK				GPIO_ResetBits(LCD_PORT, LCD_SCLK_PIN)

// Display resolution
#define LCD_X_RES                  84 // horizontal
#define LCD_Y_RES                  48 // vertical

// Cache size ( 84 * 48 ) / 8 = 504
#define LCD_CACHE_SIZE             ( ( LCD_X_RES * LCD_Y_RES ) / 8 )

// Return value
#define OK                         0   // OK
#define OUT_OF_BORDER              1   // Out of border
#define OK_WITH_WRAP               2   // Wraped
...
typedef enum
{
    FONT_1X = 0,      // Small font size
    FONT_2X = 1,      // Big font size
    FONT_4X = 2       // Large font size (fon numbert only)
} LcdFontSize;
...
static const unsigned char FontLookup [][5] =
{
   ...
   { 0x06, 0x09, 0x09, 0x06, 0x00 },   // "ГЈГ°Г Г¤ГіГ±Г»" 0x7F 127

   { 0x7C, 0x12, 0x11, 0x12, 0x7C },   // ГЂ 0xC0 192
   { 0x7F, 0x49, 0x49, 0x49, 0x31 },   // ГЃ 0xC1 193
   { 0x7F, 0x49, 0x49, 0x49, 0x36 },   // Г‚ 0xC2 194
   { 0x7F, 0x01, 0x01, 0x01, 0x01 },   // Гѓ 0xC3 195
   { 0x60, 0x3F, 0x21, 0x3F, 0x60 },   // Г„ 0xC4 196
   { 0x7F, 0x49, 0x49, 0x49, 0x41 },   // Г… 0xC5 197
   { 0x77, 0x08, 0x7F, 0x08, 0x77 },   // Г† 0xC6 198
   { 0x22, 0x41, 0x49, 0x49, 0x36 },   // Г‡ 0xC7 199
   { 0x7F, 0x10, 0x08, 0x04, 0x7F },   // Г€ 0xC8 200
   { 0x7E, 0x10, 0x09, 0x04, 0x7E },   // Г‰ 0xC9 201
   { 0x7F, 0x08, 0x14, 0x22, 0x41 },   // ГЉ 0xCA 202
   { 0x40, 0x3E, 0x01, 0x01, 0x7F },   // Г‹ 0xCB 203
   { 0x7F, 0x02, 0x0C, 0x02, 0x7F },   // ГЊ 0xCC 204
   { 0x7F, 0x08, 0x08, 0x08, 0x7F },   // ГЌ 0xCD 205
   { 0x3E, 0x41, 0x41, 0x41, 0x3E },   // ГЋ 0xCE 206
   { 0x7F, 0x01, 0x01, 0x01, 0x7F },   // ГЏ 0xCF 207
   { 0x7F, 0x09, 0x09, 0x09, 0x06 },   // Гђ 0xD0 208
   { 0x3E, 0x41, 0x41, 0x41, 0x22 },   // Г‘ 0xD1 209
   { 0x01, 0x01, 0x7F, 0x01, 0x01 },   // Г’ 0xD2 210
   { 0x07, 0x48, 0x48, 0x48, 0x3F },   // Г“ 0xD3 211
   { 0x0E, 0x11, 0x7F, 0x11, 0x0E },   // Г” 0xD4 212
   { 0x63, 0x14, 0x08, 0x14, 0x63 },   // Г• 0xD5 213
   { 0x3F, 0x20, 0x20, 0x3F, 0x60 },   // Г– 0xD6 214
   { 0x07, 0x08, 0x08, 0x08, 0x7F },   // Г— 0xD7 215
   { 0x7F, 0x40, 0x7E, 0x40, 0x7F },   // Г� 0xD8 216
   { 0x3F, 0x20, 0x3F, 0x20, 0x7F },   // Г™ 0xD9 217
   { 0x01, 0x7F, 0x48, 0x48, 0x30 },   // Гљ 0xDA 218
   { 0x7F, 0x48, 0x30, 0x00, 0x7F },   // Г› 0xDB 219
   { 0x00, 0x7F, 0x48, 0x48, 0x30 },   // Гњ 0xDC 220
   { 0x22, 0x41, 0x49, 0x49, 0x3E },   // Гќ 0xDD 221
   { 0x7F, 0x08, 0x3E, 0x41, 0x3E },   // Гћ 0xDE 222
   { 0x46, 0x29, 0x19, 0x09, 0x7F },   // Гџ 0xDF 223
   { 0x20, 0x54, 0x54, 0x54, 0x78 },   // Г  0xE0 224
   { 0x3C, 0x4A, 0x4A, 0x4A, 0x31 },   // ГЎ 0xE1 225
   { 0x7C, 0x54, 0x54, 0x28, 0x00 },   // Гў 0xE2 226
   { 0x7C, 0x04, 0x04, 0x0C, 0x00 },   // ГЈ 0xE3 227
   { 0x60, 0x3C, 0x24, 0x3C, 0x60 },   // Г¤ 0xE4 228
   { 0x38, 0x54, 0x54, 0x54, 0x18 },   // ГҐ 0xE5 229
   { 0x6C, 0x10, 0x7C, 0x10, 0x6C },   // Г¦ 0xE6 230
   { 0x00, 0x44, 0x54, 0x54, 0x28 },   // Г§ 0xE7 231
   { 0x7C, 0x20, 0x10, 0x08, 0x7C },   // ГЁ 0xE8 232
   { 0x7C, 0x21, 0x12, 0x09, 0x7C },   // Г© 0xE9 233
   { 0x7C, 0x10, 0x28, 0x44, 0x00 },   // ГЄ 0xEA 234
   { 0x40, 0x38, 0x04, 0x04, 0x7C },   // Г« 0xEB 235
   { 0x7C, 0x08, 0x10, 0x08, 0x7C },   // Г¬ 0xEC 236
   { 0x7C, 0x10, 0x10, 0x10, 0x7C },   // Г­ 0xED 237
   { 0x38, 0x44, 0x44, 0x44, 0x38 },   // Г® 0xEE 238
   { 0x7C, 0x04, 0x04, 0x04, 0x7C },   // ГЇ 0xEF 239
   { 0x7C, 0x14, 0x14, 0x14, 0x08 },   // Г° 0xF0 240
   { 0x38, 0x44, 0x44, 0x44, 0x00 },   // Г± 0xF1 241
   { 0x04, 0x04, 0x7C, 0x04, 0x04 },   // ГІ 0xF2 242
   { 0x0C, 0x50, 0x50, 0x50, 0x3C },   // Гі 0xF3 243
   { 0x08, 0x14, 0x7C, 0x14, 0x08 },   // Гґ 0xF4 244
   { 0x44, 0x28, 0x10, 0x28, 0x44 },   // Гµ 0xF5 245
   { 0x3C, 0x20, 0x20, 0x3C, 0x60 },   // Г¶ 0xF6 246
   { 0x0C, 0x10, 0x10, 0x10, 0x7C },   // Г· 0xF7 247
   { 0x7C, 0x40, 0x7C, 0x40, 0x7C },   // Гё 0xF8 248
   { 0x3C, 0x20, 0x3C, 0x20, 0x7C },   // Г№ 0xF9 249
   { 0x04, 0x7C, 0x50, 0x50, 0x20 },   // Гє 0xFA 250
   { 0x7C, 0x50, 0x20, 0x00, 0x7C },   // Г» 0xFB 251
   { 0x00, 0x7C, 0x50, 0x50, 0x20 },   // Гј 0xFC 252
   { 0x28, 0x44, 0x54, 0x54, 0x38 },   // ГЅ 0xFD 253
   { 0x7C, 0x10, 0x38, 0x44, 0x38 },   // Гѕ 0xFE 254
   { 0x48, 0x54, 0x34, 0x14, 0x7C }    // Гї 0xFF 255
};
